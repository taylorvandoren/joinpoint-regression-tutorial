...
# This file goes through the analytical procedure and data visualization of joinpoint regression all in R. 
    # There is a dedicated Joinpoint Regression Program that you can use to run the same analyses and was built
    # specifically for this regression method, and that procedure has been outlined in another file in this
...
# We will only need one package for this regression method: ljr 
    # "ljr" stands for logistic joinpoint regression. This means that these results are fit in a logistic model,
    # but we could absolutely fit the model in the dedicated Joinpoint Regression Program without this logistic transformation.
    # We could also theoretically transform the results of this analysis in R out of the logistic form, but we won't do that. 
...
# Remember, however, this package uses a logistic transformation on our data to fit these models, so instead of fitting
...
mortdata <- read.csv("mortality data.csv") # yearly mortality rate for major causes of death in US 1990-1998
pop <- read.csv("usa pop.csv") # yearly US population size 1900-1998

summary(mortdata) # the max age-specific death rate in this data is 612.4 deaths per 100,000
...
# Let's plot the data first to see what kinds of patterns we can identify without doing any modeling yet. 
...
ggplot(mortdata, aes(year, asdr, color = cod)) +
  ...
  tayloRswift::scale_color_taylor("lover") # yes, this IS a Github repo dedicated to color palettes of Taylor Swift albums

# And the same figure again with a log transformation (so we can better prepare for the following logistic analyses)

mortdata$asdrlog <- log(mortdata$asdr)
ggplot(mortdata, aes(year, asdrlog, color = cod)) +
  geom_point(size = 3.5, shape = 1) +
  geom_smooth() +
  theme_classic() +
  xlab("Year") +
  ylab("log(Age-Adjusted Death Rate (per 100,000)") +
  tayloRswift::scale_color_taylor("lover")

# Looking at this figure, we will be able to make some pretty good educated guesses about where we might find joinpoints. 
    # For example, TB declined SO MUCH during the 20th century! Where did it decline, exactly? If you want to know a lot more
    # detail about that, check out Noymer (2009) "Testing the influenza-tuberculosis selective mortality hypothesis with
    # Union Army data", Noymer (2011) "The 1918 influenza pandemic hastened the decline of tuberculosis in the United States:
    # An age, period, cohort analysis", and van Doren & Sattenspiel (2021) "The 1918 influenza pandemic did not accelerate 
    # tuberculosis mortality decline in early-20th century Newfoundland: Investigating historical and social explanations."

    # Aside from that, it looks like there was a major decline in P&I before mid-century, some decline in accidents in the 1970s,
    # and a big increase and then decrease through mid-century in heart disease, a really fascinating epidemiological phenomenon
    # that was described by Gage (2005) "Are modern environments really bad for us?: Revisiting the demographic and 
    # epidemiologic transitions. 

    # I like to start with these initial visualizations in order to get a sense of where we are going before we start going
    # there. It helps develop intuition about statistical modeling and helps us really check to see if we understand the results
    # we get. 

# Right now, mortdata is organized in a long format, but for this analysis it will be better to have it in wide format. 

mortdata.w <- mortdata %>% 
  select(year, cod, asdr)
mortdata.w <- dcast(mortdata.w, year ~ cod)
head(mortdata.w)
summary(mortdata.w)
...
mortdata.w <- data.frame(mortdata.w, exp.deaths, pop$pop)

# Let's rename all these columns to make them easier to work with

colnames(mortdata.w) <- c("year",
                          "accidents.r", "cancer.r", "heart.r", "pi.r", "stroke.r", "tb.r",  # ".r" = "rate"
                          "accidents.n", "cancer.n", "heart.n", "pi.n", "stroke.n", "tb.n",  # ".n" = "number"
                          "pop")

# Let's plot a couple real quick to make sure we have what we need

plot(mortdata.w$pi.n)
plot(mortdata.w$heart.n)
plot(mortdata.w$tb.n)

### LOGISTIC JOINPOINT MODELING ----

# Fitting the data, finding the joinpoints. 

# the 'ljr' package contains a number of built-in functions, but we will be working with only three:
#   1. ljr0() : fits zero joinpoints - just a straight line!
#   2. ljr1() : fits a single joinpoint at the most significant point of change over the time period
...
# One of the great things about these models is that the output is very short and very intuitive. They also print automatically!
...
fit.1 <- ljr0(mdw$pi.n, mdw$pop, mdw$year) 
        # arguments: number of deaths, population size (denominator of mortality rate), time
        # the output of this fit is super straightforward: we have a y-intercept of 46.75... and a slope of -0.027...
        # you can index these values easily as well
                      fit.1$Coef
                      
fit.2 <- ljr1(mdw$pi.n, mdw$pop, mdw$year, summ = T)
        # arguments: number of deaths, population size, time, automatically print summary table
        # now, we have a y-int of -15.61... and some other things!
        # g0 is the slope of the line before any joinpoints are identified. 
        # g1 max(t-tau1,0) is the slope of the line after the joinpoint is identified. 
        # we also have our first tau (joinpoint) estimate: 1918!

fit.3 <- ljrk(2, mdw$pi.n, mdw$pop, mdw$year)
        # arguments: number of joinpoints to find (k), number of deaths, population size, time
        # y-intercept: -27.9...
...
fit.4 <- ljrk(3, mdw$pi.n, mdw$pop, mdw$year)
        ...
        # You can ask the model to fit as many joinpoints as your heart desires. But you have to think about
        # whether this is a smart thing to do or not, and what it would mean to fit all those points. It would end up
        # being likely pretty meaningless. So take a look at this output and see if you can interpret it. 


# Since we want to be able to do all of this more quickly without writing four different models for every cause of death,
    # let's write a function instead that will do everything we want, including extracting those tau estimates
...
tb.calc <- jp.calc(mdw$tb.n, mdw$pop, mdw$year)
      ...
      # run them all and then make yourself some coffee - they'll be done when you get back



### MAKING SENSE OF THE MODEL FITS ----

# Remember that each model itself prints its output automatically, so by running the function that we wrote
    # with each of the models inside of it, every time you run the function you should get the four model 
    # outputs individually, along with the slope estimates for each of the segments of the fitted model and, 
    # finally, the indexable tau (joinpoint) values. This is what we will use to visualize our data. 

heart.calc
    # when I simply print the name of the object, what comes up is the list of joinpoint estimates.
...
# Before we do that, it's important to remember that although we INPUT the data as number of deaths and 
    ...
    # need to PLOT the log transformed mortality rates, and then visualize the joinpoint estimates

colnames(rates) <- c("accidents.l", "cancer.l", "heart.l", "pi.l", "stroke.l", "tb.l") # ".l" = "log"
...
# Now we'll add vertical lines where the models have estimated there are significant changes in mortality rates

# One-joinpoint model:
accidents.plot + geom_vline(xintercept = accidents.calc[[1]], size = 1.2, color = "#5C6BC0")

# Two-joinpoint model
accidents.plot + geom_vline(xintercept = accidents.calc[[2]], size = 1.2, color = "#26C6DA") 

# Three-joinpoint model
accidents.plot + geom_vline(xintercept = accidents.calc[[3]], size = 1.2, color = "#FFA726")

# One of the things that you might have noticed by now, if not by looking at the list output of the jp.calc() 
    # runs, but certainly by visualizing the data and the estimated points of change, the joinpoint estimates are
    # not necessarily the same from each fit to the next. As seen above for the accidents mortality data, there is 
    # always a joinpoint estimated somewhere around 1970, but it is never the exact same x value every single time. 
    # This is because the joinpoint estimates are highly dependent on where the preceding point is estimated, if there
